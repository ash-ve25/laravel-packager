...
class PackagerNewCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $this->info('Creating Controllers directory...');
            $this->helper->makeDir($path.$vendor.'/'.$name.'/src'.'/Controllers');
        $bar->advance();

        $this->info('Creating Models directory...');
            $this->helper->makeDir($path.$vendor.'/'.$name.'/src'.'/Models');
        $bar->advance();

        $this->info('Creating Views directory...');
            $this->helper->makeDir($path.$vendor.'/'.$name.'/src'.'/Views');
        $bar->advance();

        $this->info('Creating routes file...');
            $handle = fopen($path.$vendor.'/routes.php','w+x');
            $data = '<?php'.'\n';
            fwrite($handle, $data);
        $bar->advance();
